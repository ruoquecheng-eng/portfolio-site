  ...
  <article class="case-study">
    ...
    <section class="section netsage-screens" id="app-walkthrough">
      ...
      <div class="netsage-screen-grid">
        ...
        ${figure({depth: 2, src: "images/netsage-app-samples.webp", width: 1080, height: 2400, alt: "NetSage sample center with offline DNS, connection, TLS, HTTP, and packet loss samples", caption: "Sample center. Fourteen bundled, offline examples support walkthroughs and repeatable rule-engine checks.", className: "netsage-screen"})}
        ${figure({depth: 2, src: "images/netsage-app-scenarios.webp", width: 1080, height: 2400, alt: "NetSage fault scenario library grouped into DNS, connection, TLS, HTTP, and packet loss cases", caption: "Scenario library. Ten offline troubleshooting scenarios can be sent into the diagnostic workflow.", className: "netsage-screen"})}
        ...
        ${figure({depth: 2, src: "images/netsage-app-combined.webp", width: 1080, height: 2400, alt: "NetSage combined diagnosis form that combines an optional log with a user-selected HTTPS target", caption: "Combined diagnosis. A user-confirmed target and optional log evidence are retained in one local session.", className: "netsage-screen"})}
        ${figure({depth: 2, src: "images/netsage-app-active-result.webp", width: 1080, height: 2400, alt: "NetSage active quick check result showing an Android emulator network snapshot and successful DNS lookup for example.com", caption: "Active check result. The on-device run records its network snapshot and DNS evidence; it is a functional emulator check, not a field-performance claim.", className: "netsage-screen"})}
        ${figure({depth: 2, src: "images/netsage-app-log-result.webp", width: 1080, height: 2400, alt: "NetSage local diagnostic result for a bundled DNS NXDOMAIN sample with evidence and repair steps", caption: "Rule-based log result. A bundled DNS sample shows ranked evidence, a bounded rule score, and three repair steps.", className: "netsage-screen"})}
        ${figure({depth: 2, src: "images/netsage-app-history.webp", width: 1080, height: 2400, alt: "NetSage on-device diagnostic session history showing a retained local DNS result and its actions", caption: "Diagnostic sessions. Inputs, evidence, top hypotheses, and retest differences remain on the device; this capture shows one bundled DNS session.", className: "netsage-screen"})}
        ${figure({depth: 2, src: "images/netsage-app-reference.webp", width: 1080, height: 2400, alt: "NetSage offline error code and terminology reference with category filters and suggested actions", caption: "Error-code reference. Searchable local entries connect common terms and error codes to concise handling suggestions.", className: "netsage-screen"})}
        ${figure({depth: 2, src: "images/netsage-app-feature-map.webp", width: 1080, height: 2400, alt: "NetSage feature overview listing diagnosis, checks, samples, scenarios, session history, and local tools", caption: "Feature overview. The app keeps its primary diagnosis, reference, storage, and local-tool entry points in one place.", className: "netsage-screen"})}
      </div>
    </section>
    ...
    <section class="section evidence-section" id="evidence">
      ...
      <dl class="evidence-ledger">
        ...
        <div><dt>Published visual</dt><dd>Eleven Android screenshots captured from the current v0.2.0 build, the application icon, and an original conceptual diagnostic flow</dd></div>
      </dl>
    </section>
...
const researchClaims = new Map([
  [connected, { status: connectedStatus, venue: connected.journal, summary: connectedSummary, access: connectedAccess, authorship: "Single-author manuscript", byline: profileName }],
  [cubic, { status: cubicStatus, venue: cubic.journal, summary: cubicSummary, access: cubicAccess, authorship: "Single-author manuscript", byline: profileName }],
  [hypergraph, { status: hypergraphStatus, venue: hypergraphVenue, summary: hypergraphSummary, authorship: "Six-author manuscript · Wanzheng Ning is first and corresponding author", byline: hypergraph.authorshipText }]
]);
const researchStatusFor = (record) => researchClaims.get(record)?.status ?? "";
const researchVenueFor = (record) => researchClaims.get(record)?.venue ?? "";
...
const resumeBody = `
  <article class="resume">
    ...
    <section class="resume-section"><h2>Projects</h2><div class="resume-entry"><div><strong>NetSage</strong></div><p>Built a serverless Android network-diagnostics workbench combining local log rules with active probes, explainable evidence, retesting, session history, and Markdown/JSON export.</p></div><div class="resume-entry"><div><strong>CommLab</strong></div><p>Built a local-first research platform with 130 interactive laboratory modes spanning physical-layer communications, ISAC, distributed learning, resilient edge inference, networked control, and robust offline policy evaluation.</p></div><div class="resume-entry"><div><strong>Battery RUL and cascade utilization modeling</strong></div><p>Q1 to Q4 workflow on fully simulated data generated with semi-empirical assumptions, covering degradation stages, SOH/RUL, compatibility graphs, MILP grouping, and robust stress testing; awarded a university-level second prize.</p></div><div class="resume-entry"><div><strong>Australian high-speed rail design and management concept</strong></div><p>Contributed to a lightweight composite carriage concept and independently developed a five-page front-end prototype for capital pooling, risk simulation, compliance workflow, and impact reporting using illustrative data.</p></div></section>
    <section class="resume-section"><h2>Competitions</h2><div class="resume-entry resume-radio"><div><strong><a href="${local(1, 'projects/radio-localization/')}">${escapeHtml(radio.type)} — ${escapeHtml(radio.title)}</a></strong></div><p>Competition modeling of interference-source localization and clearance using bounded-error geometry, robust active sensing, and rolling route planning; all sources cleared in six official simulator runs.</p></div><div class="resume-entry"><div><strong>Scenic Guide Digital Human</strong></div><p>Tourism guide prototype to which I contributed, with conversational and voice interaction, route guidance, narration, and knowledge management.</p></div></section>
    ...
    <section class="resume-section"><h2>Languages</h2><p>Chinese · English</p></section>
